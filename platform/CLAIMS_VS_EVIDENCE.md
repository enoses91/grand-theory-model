...
**Status:** living document · **Version:** 0.3 (Claude draft for Perplexity refinement) · **Date:** 2026-06-28 · **Owner:** Eric Enos
...
**v0.3 change:** added E12/E13 (recursive self-application: productive-frame selection as proceduralizable; the EVR trail as the unrecorded calibration signal — "not yet" not "not available") and M8 (the learnable-transferable / learnable-personal / not-learnable trichotomy determines the product shape), pairing with research program Part VII. Added analogy/frame-selection citation block.
...
| C8 | The "ping" (productive-antecedent recognition) is recognition-primed pattern-matching in the *abstraction* domain — learned, not mystical | UNTESTED | Structure-mapping (Gentner); RPD (Klein, C5) applied to abstraction; analogy-as-cognition (Hofstadter & Sander) `[mem]` | May conflate distinct faculties; "learned recognizer" is a hypothesis, not established for *abstraction* selection specifically | L-M | Pull Gentner structure-mapping + Gick & Holyoak analogical-transfer; find evidence on whether productive-analogy recognition is trainable/expertise-linked (supports Part VII) |
...
| E12 | **Productive-frame selection** (picking the abstraction level at which an analogy is load-bearing, not vacuous) is proceduralizable / algorithmically searchable — not an uncodifiable flash | UNTESTED | Generation of analogies is near-solved (LLMs); the open part is *selection*. Structure-mapping (Gentner) gives a formal target; the generator–critic–triage architecture (Part VII.3) is a concrete proposal `[mem]` | Any two things rhyme at a high-enough abstraction → **vacuous over-generalization** is the failure mode; an LLM critic may confidently bless vacuous frames (E2/E11 at the meta level); the relevance-triage front-end has the least prior art | L | Find work on automated analogical reasoning + frame/abstraction selection; whether LLM critics discriminate productive vs vacuous abstractions at human precision (the Part VII.7 calibration experiment) |
| E13 | The calibration signal for productive-frame selection (*which* pursued abstractions proved load-bearing vs vacuous) is **capturable** by the EVR/provenance trail — "not available" → "not yet" | UNTESTED | The signal is sparse/delayed/unrecorded in the wild; but the platform's provenance + coherence/rework machinery (III.5/III.6) can log proposed→pursued→proved-load-bearing in normal use `[I]` | Speculative `[A]`: the back-labeling (did this generalization prove load-bearing?) may itself be noisy/subjective; capture ≠ learnability (see M8) | L | n/a literature primarily — this is a design/instrumentation claim (Part VII.4/VII.7). Perplexity: find any precedent for logging analyst judgment-outcomes as training data (process mining; learning-from-demonstration) |
...
| M8 | The product *shape* is decided by which of three outcomes holds: productive-frame discrimination is learnable-and-transferable / learnable-but-personal / not-learnable-from-outcomes | UNTESTED | Trichotomy is logically exhaustive; "learnable-but-personal" maps to the rubric's empowerment buyer-shape pointed at *judgment* (apprentices to one operator, amplifies them) `[I]` | Can't be resolved without the E13 captured dataset; lean is "personal" (most likely, most interesting) but that is `[A]` | L | n/a retrieval — resolved by the Part VII data program, not literature. Perplexity: find precedents for personalized-vs-transferable expertise models (recommender personalization; apprenticeship learning) |
...
6. **E12 + E13 + M8 (recursive self-application)** — *off* the critical kill-path and longest-horizon, but **E13's capture must begin early** (the dataset accrues only with use). E12/M8 cannot resolve until the data exists. Treat as: instrument now (cheap), analyze later. Inherits E2/E11/III.1 — if those null, this collapses with them. Standing guard: the conversations that motivate this are single-instance with no control arm; do not treat them as evidence the loop automates.
...
- **Analogy / frame selection (C8, E12; Part VII):** Gentner (1983), structure-mapping theory; Gentner & Markman, structural alignment `[mem]`; Gick & Holyoak (1980/1983), analogical transfer & the spontaneous-transfer failure `[mem]`; Holyoak & Thagard, *Mental Leaps* `[mem]`; Hofstadter & Sander, *Surfaces and Essences* `[mem]`; Kolodner, case-based reasoning `[mem]`

---

## 7. Reflexive note

This register is itself the system's method applied to the system: enumerate the claims (dimensions), force the disconfirming evidence into frame (not just the confirming), mark uncertainty explicitly rather than fill it, and track which claims later claims depend on (E3 depends on C1–C4; M4 depends on M-claims; E11 is the honest guard on E10/M7; E12/E13/M8 inherit E2/E11/III.1 and must not be read as self-proving from the conversations that generated them). Maintaining it *is* a hand-run instance of the coherence-across-a-series discipline the product is meant to automate — and, per Part VII, a candidate *source of the very calibration data* that discipline would need to automate its own hardest step. `[I]`
