...
**Status:** living document · **Version:** 0.1 (Claude draft for Perplexity refinement) · **Date:** 2026-06-28 · **Owner:** Eric Enos
...
| E8 | Tracking coherence across a decision-*series* reduces drift (the differentiator) | UNTESTED | None — atomic-decision literature structurally cannot speak to it (this is the point) | Absence of literature = opportunity OR unstudied-because-intractable | L-M | Find anything on cross-decision consistency / commitment-drift in longitudinal decision programs (strategy process lit — Mintzberg; design rationale — IBIS); likely sparse |
| E9 | Early drift detection avoids downstream rework (the most quantifiable value claim) | UNTESTED | Logical from dependency structure | — | L-M | Find rework-cost literature (software change-impact analysis; engineering change propagation — Clarkson et al.); borrow metrics |
...
This register is itself the system's method applied to the system: enumerate the claims (dimensions), force the disconfirming evidence into frame (not just the confirming), mark uncertainty explicitly rather than fill it, and track which claims later claims depend on (E3 depends on C1–C4; M4 depends on M-claims). Maintaining it *is* a hand-run instance of the coherence-across-a-series discipline the product is meant to automate. `[I]`
